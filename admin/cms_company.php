...
if (isset($_POST["updateCompany"])) {
    if (!empty(trim($_POST["id"])) && !empty(trim($_POST["name"])) && !empty(trim($_POST["companyCat"])) && !empty(trim($_POST["postcode"])) && !empty(trim($_POST["address"]))) {
        $id        = $_POST["id"];
        $name      = $_POST["name"];
        $cat       = $_POST["companyCat"];
        ...
        $editLifestyle = $link->query("UPDATE company_info SET company_name='$name', company_category='$cat', company_brief = '$brief', company_latitude = '$latitude', company_longitude = '$longitude', address = '$addr', postal = '$postal' WHERE company_id = '$id'");
        if ($editLifestyle) {
...
if (isset($_POST["newCompany"])) {
    if (!empty(trim($_POST["name"])) && !empty(trim($_POST["companyCat"])) && !empty(trim($_POST["postcode"])) && !empty(trim($_POST["address"]))) {
        $name      = $_POST["name"];
        $cat      = $_POST["companyCat"];
        $postal    = $_POST["postcode"];
        $addr      = $_POST["address"];
        ...
        $addLifestyle = $link->query("INSERT INTO company_info (company_name, company_category, company_brief, company_latitude, company_longitude, address, postal) VALUES ('$name', '$cat', '$brief', '$latitude', '$longitude', '$addr', '$postal')");
        if ($addLifestyle) {
...
if ($company) {
    $company_table = "<table class='table table-sm cmsTables'><thead><tr class='bg-primary'><th>ID</th><th>Name</th><th>Company Category</th><th>Address</th><th>Postal</th><th>Latitude</th><th>Longitude</th><th>Brief</th><th></th><th></th></tr></thead><tbody><br>";
    while ($row = mysqli_fetch_array($company)) {
        $company_table .= "<tr><td>" . $row["company_id"] .
                          "</td><td>" . $row["company_name"] .
                          "</td><td>" . $row["company_category"] .
                          "</td><td>" . $row["address"] .
                          "</td><td>" . $row["postal"] .
    ...
    <form action="cms_company.php" method="post">
        <div class="form-row">
            ...
            <div class="form-group col-md-5">
                ...
                <input type="text" class="form-control" name="companyCat" id="companyCat" />
            </div>
        </div>
        <div class="form-row">
